...
describe('TvShowsComponent', () => {
  ...
  beforeEach(async () => {
    ...
    const routerMock = () => ({ navigateByUrl: _arg => ({}) });
  ...
  // });
  it(`tvShowsList has default value`, () => {
    expect(component.tvShowsList).toEqual([]);
  });
  it(`showsRating has default value`, () => {
    expect(component.showsRating).toEqual([]);
  });
  ...
  it(`should navigate to shows details page`, () => {
    ...
    component.navigateToShowDetail('tvshowDetails');
      expect(mockRouter.navigate).toHaveBeenCalledWith('/tvshowDetails');
  });
  it(`should compare two values`, () => {
    fixture =  TestBed.createComponent(TvShowsComponent);
    component = fixture.componentInstance;
    let sorted = component.compareFunction(showRatingsArray[0],showRatingsArray[1]);
    expect(sorted).toEqual(jasmine.arrayContaining(expectedShowRatingsArray));
  });
});
